...
<body class="bgbody">
	...
	<div id="content-wrapper">
		...
		${if:groups}
			<tr>
				<td><strong>${msg:presurvey.department}</strong><br /><select name="group">${page:groups}</select></td>
			</tr>
		${endif:groups}
			<tr>
				<td><strong>${msg:presurvey.name}</strong><br /><input type="text" name="name" size="50" value="${form:name}" class="username" ${ifnot:showname}disabled="disabled"${endif:showname}/></td>
			</tr>
		${if:showemail}
			<tr>
				<td><strong>${msg:presurvey.mail}</strong><br /><input type="text" name="email" size="50" value="${form:email}" class="username"/></td>
			</tr>
		${endif:showemail}
		${if:showmessage}			
			<tr>
				<td valign="top"><strong>${msg:presurvey.question}</strong><br /><textarea name="message" tabindex="0" cols="45" rows="2">${form:message}</textarea></td>
			</tr>
		${endif:showmessage}			
                ${if:showcaptcha}
                        <tr>
                                <td><img src="captcha.php"/><br /><input type="text" name="captcha" size="50" maxlength="15" value="" class="username"/></td>
                        </tr>
                ${endif:showcaptcha}
		</table>		
